...
def ball_collision_check(ball1, ball2):
    ...
    # each other
    return distance_less_equal(ball1.get_pos(), ball2.get_pos(), 2 * config.ball_radius) and \
           np.count_nonzero(np.concatenate((ball1.get_velocity(), ball2.get_velocity()))) > 0 and \
           np.dot(ball2.get_pos() - ball1.get_pos(), ball1.get_velocity() - ball2.get_velocity()) > 0


def collide_balls(ball1, ball2):
    point_diff = ball2.get_pos() - ball1.get_pos()
    dist = point_distance(ball1.get_pos(), ball2.get_pos())
    # normalising circle distance difference vector
    collision = point_diff / dist
    # projecting balls velocity ONTO difference vector
    ball1_dot = np.dot(ball1.get_velocity(), collision)
    ball2_dot = np.dot(ball2.get_velocity(), collision)
...
def line_ball_collision_check(line, ball):
    ...
    if distance_less_equal(line.middle, ball.get_pos(), line.length / 2 + config.ball_radius):
        # displacement vector from the first point to the ball
        displacement_to_ball = ball.get_pos() - line.line[0]
        ...
        return -config.ball_radius / 3 <= projected_distance <= \
               ...
               np.hypot(*(closest_line_point - ball.get_pos() + line.line[0])) <= \
               config.ball_radius and np.dot(
            perpendicular_vector, ball.get_velocity()) <= 0
...
def collide_line_ball(line, ball):
    ...
    ball.velocity -= 2 * np.dot(perpendicular_vector,
                                ball.get_velocity()) * perpendicular_vector
